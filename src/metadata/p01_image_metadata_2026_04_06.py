...
def parse_header(h: np.ndarray) -> dict:
    ...
    # Guard: lua_timestamp of 0 or > year 3000 (ms) means corrupt/pre-fix header.
    _MAX_TS_MS = 32503680000000  # 3000-01-01 in Unix ms
    if lua_timestamp > _MAX_TS_MS:
        raise ValueError(
            f"lua_timestamp={lua_timestamp} is invalid — the .bin file may have "
            "been generated by an older z03 with a JSON header. "
            "Re-run z03 to produce a new file."
        )
