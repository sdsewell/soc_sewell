...
Inputs: 2027-01-01T00-00-00Z_science_ROI_L1.1.npy (or equivalent) generated by running load_image.py
Output-1: 2027-01-01T00-00-00Z_science_ROI_L1.1_center.png
Output-2: 2027-01-01T00-00-00Z_science_ROI_L1.1_center.npz (the new seed used by annular_reduction.py)
...
def main() -> None:
    ...
    image = np.load(src, allow_pickle=True)
    ...
    # ── Save centre result ────────────────────────────────────────────────────
    centre_path = src.with_name(src.stem + "_center.npz")
    np.savez(
        centre_path,
        cx       = np.array(cr.cx),
        cy       = np.array(cr.cy),
    ...
    fig.suptitle(
        ...
        f"ROI {H}×{W} px  |  "
        f"seed: cx = {cx_seed:.1f},  cy = {cy_seed:.1f} px  (image centre)  |  "
        f"grid cost = {cr.grid_cost:.4g}  →  NM cost = {cr.cost_at_min:.4g}",
        fontsize=11, fontweight="bold",
    )
    fig.tight_layout()
    png_path = src.with_name(src.stem + "_center.png")
