"""
Script:  H06_airglow_inversion_2026_05_12.py
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  Inversion stages:
    Stage 1 — Brute-force scan of λ_c over ±1.5 FSR (200 grid points)
...
# FSR at OI 630 nm (used for scan range and Jacobian step)
...
def _lambda_c_scan(r_good, prof_good, sigma_good, r_max, cal,
                   n_scan=200, n_fine=500):
    """
    Scan λ_c over ±1.5 FSR around OI_WAVELENGTH_AIR_M.
    At each point, analytically solve for Y_line and B_sci (linear given λ_c).

    Returns (lambda_c_best, chi2_min, scan_ambiguous_flag).
    Covers cross-track (offset=0) and along-track (offset=−1, −2) regimes.
    """
    lc_lo = OI_WAVELENGTH_AIR_M - 1.5 * FSR_OI_M
    lc_hi = OI_WAVELENGTH_AIR_M + 1.5 * FSR_OI_M
...
def _run_lm(r_good, prof_good, sigma_good, r_max, cal,
            lc_init, Y_init, B_init, n_fine=500):
...
def _compute_covariance(r_good, prof_good, sigma_good, r_max, cal,
                        lc, Y_line, B_sci, chi2_red, n_fine=500):
    """
    Compute 3×3 covariance for [λ_c, Y_line, B_sci] via FD Jacobian.

    Uses step = FSR_OI_M/1000 ≈ 4.7 m/s for λ_c — avoids the ≈1-FSR
...
def make_figure(r2_data, profile, sigma,
    ...
    forder        = int(round((lc_m - OI_WAVELENGTH_AIR_M) / FSR_OI_M))
    ...
    ax_tbl.text(0.01, 0.01,
        ...
        f"FSR_OI = {FSR_OI_M*1e15:.2f} fm   "
        f"v_FSR ≈ {SPEED_OF_LIGHT_MS*FSR_OI_M/OI_WAVELENGTH_AIR_M:.0f} m/s   "
...
def main():
    ...
    # ---- 6. Stage 1: λ_c scan ----
    print("\nStage 1 — brute-force λ_c scan (±1.5 FSR, 200 points)…")
