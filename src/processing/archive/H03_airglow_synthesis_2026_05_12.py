"""
Script:  H03_airglow_synthesis_2026_05_12.py
Purpose: Generate a synthetic OI 630 nm airglow fringe image using the
         ...
         and optionally save the image and 1D profile as .npy files.
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  OI line intensity Y_line (ADU / wavelength bin)
    Typical quiet-time airglow: 500–2000 ADU
    Default: 1000 ADU

  Sky background Y_bg (ADU / wavelength bin)
    Default: 0 (pure line, no background)

  SNR (peak-to-trough / noise σ)
    Default: 5.0  (Harding §4 Monte Carlo baseline)
    Typical real data: 2–10
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  Bottom: Pixel value histogram of the image.
          Shows noise distribution; for Gaussian noise should be
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  files and can be loaded directly by H06_airglow_inversion_2026_05_12.py.

Run from repo root:
    python src/processing/H03_airglow_synthesis_2026_05_12.py
"""
...
def _ask_source_and_noise(root):
    ...
    snr = simpledialog.askfloat(
        "SNR",
        "Signal-to-noise ratio: SNR = ΔS / σ_N\n"
        "where ΔS = peak−trough of fringe profile.\n"
        "Harding (2014) baseline: 5.0",
        initialvalue=_DEFAULTS['snr'], minvalue=0.1, maxvalue=100.0,
        parent=root) or _DEFAULTS['snr']

    return dict(Y_line=Y_line, Y_bg=Y_bg, snr=snr)


def make_figure(result: dict, params: InstrumentParams,
                obs_mode, v_rel_ms: float, snr: float) -> plt.Figure:
    """
    ...
    ax_his.set_title("Pixel value histogram  (Gaussian noise → approximately Gaussian)",
                     fontsize=9)
...
def main():
    ...
    result = synthesise_airglow_image(
        ...
        noise_type       = _DEFAULTS['noise_type'],
        snr              = src['snr'],
    ...
    # ---- Figure ----
    fig = make_figure(result, params, obs_mode, v_rel_ms, src['snr'])
    plt.show()
