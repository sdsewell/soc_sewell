...
Changes from prior version (m05_calibration_inversion_2026_04_06.py):
...
class FitConfig:
    ...
    When tolansky is None:
      t_init   = self.t_init_m or ETALON_GAP_M   (FSR-ambiguity risk)
      alpha_init = self.alpha_init or ALPHA_RAD_PX  (from windcube/constants.py)
      TOLANSKY_NOT_PROVIDED quality flag is set.
    """
    ...
    def resolve(self, profile) -> dict:
        ...
        Group A (Tolansky-seeded):
          If tolansky provided: t_init = tolansky.d_m; alpha_init = tolansky.alpha_rad_px.
          Else: t_init = self.t_init_m or ETALON_GAP_M; alpha_init = self.alpha_init or ALPHA_RAD_PX.
        Group B (fringe-shape): R, I0, I1, I2, sigma0, sigma1, sigma2, B use instrument defaults.
        """
        r = {}

        # --- t_m (Group A — Tolansky-seeded) ---
        ...
        # --- R_refl (Group B — fitted from fringe shape) ---
        ...
        # --- I0 and B (Group B): auto-initialise from profile if available ---
...
class CalibrationResult:
    ...
    Fitted parameters fall into two groups (H05 §2):
    - Group A (Tolansky-seeded, fine-tuned with tight bounds): t_m, alpha
    - Group B (fitted from fringe shape, no Tolansky prior):
        R_refl, I0, I1, I2, sigma0, sigma1, sigma2, B
    """
    # --- Fitted parameters (10 total) ---
    # Group A: Tolansky-seeded, fine-tuned with tight bounds
    t_m:     float   # etalon gap (m); seeded by Tolansky d_m, ±20 µm bounds
    R_refl:  float   # Group B — effective plate reflectivity (—)
    alpha:   float   # Group A — plate scale (rad/px); seeded by Tolansky alpha_rad_px, ±5% bounds
    # Group B: fitted from fringe shape (no Tolansky prior)
    ...
    epsilon_cal:           float   # (2*t_m / NE_WAVELENGTH_1_M) % 1; zero-wind reference for H06
    ...
    # --- Stage diagnostics ---
    chi2_by_stage: list   # [chi2_s1, chi2_s2, chi2_s3, chi2_s4]; monotone non-increasing
...
def _stage0_seed(profile, config):
    ...
    Group A (t, alpha): seeded from Tolansky if provided, else from
    ETALON_GAP_M / ALPHA_RAD_PX (windcube/constants.py). Not computed
    from FOCAL_LENGTH_M or CCD pixel pitch.
