...
def load_images() -> dict:
    ...
    def _load_file(path):
        ...
        if suffix == ".bin":
            ...
            FULL_SIZE     = 260 * 276 * 2   # 143 520 — binary header embedded
            ANALYZED_SIZE = 259 * 276 * 2   # 142 968 — headerless + JSON sidecar

            if len(raw_bytes) == FULL_SIZE:
                meta, pixels = ingest_real_image(path)
                raw = np.frombuffer(raw_bytes, dtype=">u2").reshape(260, 276)
                return pixels.astype(np.float64), raw, meta, "real"

            elif len(raw_bytes) == ANALYZED_SIZE:
                # Headerless format: pixel data only; metadata lives in a .txt sidecar
                pixels = np.frombuffer(raw_bytes, dtype=">u2").reshape(259, 276)
                sidecar = path.with_suffix(".txt")
                meta = None
                if sidecar.exists():
                    d = json.loads(sidecar.read_text())
                    # Normalise legacy quaternion typo
                    if "attitude_quadternion" in d:
                        d["attitude_quaternion"] = d.pop("attitude_quadternion")
                    # Fields present in the JSON sidecar may differ from the full
                    # binary header; fill in defaults for anything missing.
                    meta = ImageMetadata(
                        rows                 = d.get("rows", 260),
                        cols                 = d.get("cols", 276),
                        exp_time             = d.get("exp_time", 0),
                        exp_unit             = d.get("exp_unit", 0),
                        binning              = 2 if d.get("cols", 276) == 276 else 1,
                        img_type             = d.get("img_type", "science"),
                        lua_timestamp        = d.get("lua_timestamp", 0),
                        adcs_timestamp       = d.get("adcs_timestamp", 0),
                        utc_timestamp        = d.get("utc_timestamp", ""),
                        spacecraft_latitude  = d.get("spacecraft_latitude", 0.0),
                        spacecraft_longitude = d.get("spacecraft_longitude", 0.0),
                        spacecraft_altitude  = d.get("spacecraft_altitude", 0.0),
                        pos_eci_hat          = d.get("spacecraft_position", [0.0, 0.0, 0.0]),
                        vel_eci_hat          = d.get("spacecraft_velocity", [0.0, 0.0, 0.0]),
                        attitude_quaternion  = d.get("attitude_quaternion", [0.0, 0.0, 0.0, 1.0]),
                        pointing_error       = d.get("pointing_error", [0.0, 0.0, 0.0, 1.0]),
                        obs_mode             = d.get("obs_mode", "unknown"),
                        ccd_temp1            = d.get("ccd_temp1", 0.0),
                        etalon_temps         = d.get("etalon_temps", [0.0, 0.0, 0.0, 0.0]),
                        shutter_status       = d.get("shutter_status", "unknown"),
                        gpio_pwr_on          = d.get("gpio_pwr_on", [0, 0, 0, 0]),
                        lamp_ch_array        = d.get("lamp_ch_array", [0, 0, 0, 0, 0, 0]),
                        lamp1_status         = d.get("lamp1_status", "off"),
                        lamp2_status         = d.get("lamp2_status", "off"),
                        lamp3_status         = d.get("lamp3_status", "off"),
                        is_synthetic         = False,
                    )
                return pixels.astype(np.float64), None, meta, "real"

            else:
                raise ValueError(
                    f"Unexpected .bin file size {len(raw_bytes)} bytes for {path.name}. "
                    f"Expected {FULL_SIZE} (header-embedded) or {ANALYZED_SIZE} (headerless)."
                )
