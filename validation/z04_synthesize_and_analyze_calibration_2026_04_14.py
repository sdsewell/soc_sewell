...
def main(argv=None):
    ...
    # Launch Z01 analysis on the freshly written files.
    z01_path = pathlib.Path(__file__).resolve().parent / "z01_analyze_cal_dark_pair_2026_04_14.py"
    if z01_path.exists():
        print(f"\nLaunching Z01: {z01_path.name} ...")
        env = os.environ.copy()
        env["Z01_CAL_PATH"]  = str(path_cal)
        env["Z01_DARK_PATH"] = str(path_dark)
        subprocess.run([sys.executable, str(z01_path)], env=env)
    else:
        print(f"\nWarning: Z01 not found at {z01_path}")
