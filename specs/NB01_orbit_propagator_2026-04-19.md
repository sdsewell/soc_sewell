...
> **Revision note (2026-04-19) — sgp4 2.x API fix:**
> `_datetime_to_sgp4_epoch()` and `sgp4init()` call corrected for sgp4 2.x.
> See §5.3 and the new §8 (Known Pitfalls) for details. All 8 tests still pass.

> **Revision note (same-day post-implementation, 2026-04-16):**
...
#### `_datetime_to_sgp4_epoch(dt) → float`

Converts a UTC datetime to the SGP4 2.x internal epoch format: a single
float equal to `Julian Date − 2433281.5`.

**sgp4 2.x requires a single float.** The sgp4 1.x API used a
`(year, fractional_doy)` tuple — that form must not be used with sgp4 >= 2.0.

```python
def _datetime_to_sgp4_epoch(epoch_dt) -> float:
    """
    Convert a UTC datetime to the SGP4 2.x internal epoch float.

    Returns
    -------
    float
        Julian Date minus 2433281.5, the SGP4 internal time baseline
        (days since 1949-12-31 00:00 UT).

    Notes
    -----
    sgp4 1.x took (year: int, epochdays: float) as two arguments.
    sgp4 2.x takes a single float: JD - 2433281.5.
    Passing the old tuple form to sgp4 2.x does NOT raise an exception —
    it silently sets the epoch to year 1 AD, causing T6 to fail.
    """
    from astropy.time import Time
    t = Time(epoch_dt, scale='utc') if not isinstance(epoch_dt, Time) else epoch_dt
    return t.jd - 2433281.5
```
...
The `sgp4init()` call must use:
- Argument 2: `'i'` (opsmode, required string in sgp4 2.x)
- Argument 4: single float from `_datetime_to_sgp4_epoch()` — **not** a tuple

```python
satrec.sgp4init(
    WGS84,                              # gravity model
    'i',                                # opsmode — required by sgp4 2.x
    0,                                  # satellite number (arbitrary)
    _datetime_to_sgp4_epoch(epoch_dt),  # single float: JD − 2433281.5
    bstar,                              # drag term B*
    0.0,                                # ndot
    0.0,                                # nddot
...
**SGP4 unit trap.** The `sgp4` library returns position in **km** and velocity
in **km/s**. Convert to metres and m/s immediately after every SGP4 call:
...
**sgp4 version API break — epoch format.**
The `sgp4` library changed its `sgp4init()` epoch argument between versions:

| sgp4 version | Epoch argument(s) | Form |
|---|---|---|
| 1.x (old) | args 4 and 5 | `year: int, epochdays: float` (two args) |
| 2.x (current) | arg 4 only | single `float`: `JD − 2433281.5` |

Passing a `(year, epochdays)` tuple to sgp4 2.x does **not** raise an
exception. It silently sets the epoch to year 1 AD and propagates without
error. The symptom is that test T6 fails — the `epoch` column reads as
year 0001 UTC while ECI vectors look superficially correct.

Always use `_datetime_to_sgp4_epoch()` from this module, which returns the
correct single float. Do not rewrite it to return a tuple.

Verify your installed version:
```bash
python -c "import sgp4; print(sgp4.__version__)"   # must be >= 2.0
```

**sgp4init opsmode argument.** Argument 2 of `sgp4init` must be the string
`'i'` (improved mode). Omitting it or passing `None` raises a `TypeError` in
sgp4 2.x. This argument did not exist in sgp4 1.x.

**TEME vs ECI J2000.** SGP4 natively outputs in TEME (True Equator, Mean
Equinox), not ECI J2000/GCRS. The difference is < 1 arcsec (~50 m at
500 km altitude) and is negligible for this application. A comment in the
code flags this approximation. If sub-metre precision is needed in future,
apply the TEME→GCRS rotation from astropy before storing.

**Frame confusion.** The ECI vectors produced here are not in BRF, THRF, or
SIRF. Do not pass `pos_eci_*` columns into any function expecting a BRF-frame
vector without first applying the appropriate rotation. See §2.4 for the
frame hierarchy. The `_eci` suffix in column names is the authoritative
frame indicator.
...
## 10. Output contract for downstream modules
...
## 11. File location in repository
