# H05 — Staged Calibration Inversion

**Spec ID:** H05
**Spec file:** `docs/specs/H05_calibration_inversion_2026-05-05.md`
...
**Used by:** H06 (airglow inversion receives CalibrationResult),
S17 (INT02), S18 (INT03)
**Last updated:** 2026-05-05

> **What changed from 2026-04-06:**
> 1. **`FOCAL_LENGTH_M` removed** from the constants import block (Section 4).
>    `alpha` (rad/px) is the fundamental plate-scale parameter used throughout
>    this module and is imported directly from `ALPHA_RAD_PX`. Focal length and
>    pixel pitch are its physical origin but play no role here.
> 2. **`t` and `alpha` treatment clarified** (Sections 2 and 9). Both parameters
>    arrive from Tolansky as seeds with tight bounds; they are not free parameters
>    in the usual sense. The fitted-parameter table now distinguishes
>    "Tolansky-seeded, fine-tuned with tight bounds" from "Freely fitted from
>    fringe shape". This is consistent with H01 §4.2 (updated 2026-05-05).
> 3. **References updated** throughout: S09/M01 → H01; spec ID corrected from
>    S14 to H05.
> 4. **File locations updated** to reflect new date suffix.
> 5. No changes to the fitting algorithm, stage sequence, bounds architecture,
>    dataclasses, or tests.

---

## 1. Purpose

H05 recovers the WindCube FPI instrument parameters from the neon
...
**Relationship to H01:** H05 is the inversion driver for the H01 forward
model. H01 defines `build_instrument_matrix()`, `InstrumentParams`, and
`make_ne_spectrum()`. H05 calls these to evaluate the forward model at each
LM iteration and drives the optimisation. H05 has no forward-model
mathematics of its own — all physics is in H01.

The parameters recovered by H05 fall into two groups:

**Group A — Tolansky-seeded geometry** (arrive as priors, fine-tuned in
Stage 2 with tight Tolansky-derived bounds):

| Symbol | Description | Units |
|--------|-------------|-------|
| `t`    | Etalon plate spacing | m |
| `alpha`| Plate scale (rad/px) | rad/px |

**Group B — Fitted from fringe shape** (no Tolansky prior; determined
entirely by the amplitude and width of the fringe peaks):
...
science Doppler shifts are subsequently measured.

---

## 2. The role of Tolansky priors

### 2.1 Why Tolansky runs first

The H01 Airy function is periodic in `t` with period FSR = λ²/(2nt).
Without an external constraint on `t`, the brute-force scan over the
full parameter space would span thousands of FSR periods, making
convergence unreliable. Tolansky resolves this by measuring the beat
period between the two neon lines, placing `t` within the correct FSR
period to < 1 µm accuracy before any LM fitting begins.

Similarly, `alpha` (the plate scale) controls fringe spacing. Tolansky
recovers `alpha` from the spacing of fringe peak positions to ~1–3% accuracy,
providing a tight initial seed that eliminates the (t, α) valley described
in Section 3.1.

### 2.2 What Tolansky provides

S13 (`TolanskyPipeline`) runs on the same `FringeProfile` and recovers,
from peak positions alone:

| Tolansky output | H05 use | Accuracy |
|----------------|---------|----------|
...
period with < 1 µm accuracy, the brute-force scan over `t` (used in older
implementations) is **entirely eliminated**. The full 4-stage sequence begins
with `t` already correct.

**Why `t` and `alpha` still appear in the LM fit despite Tolansky seeds:**
Tolansky uses peak *positions* only — not the full fringe shape. The LM
fit with tight bounds allows `t` and `alpha` to move by a small amount
within those bounds in order to minimise χ² over the full fringe shape,
including the wings of each peak. The result is a refined estimate with
full uncertainty quantification from the covariance matrix. The Tolansky
values are starting points and tight constraints, not final answers.
...
These are the exclusive province of H05.
...
ratio `λ²/(nd)`. A small increase in `t` can be exactly compensated by a
...
## 4. Physical constants from H01 / `windcube/constants.py`

All constants are imported from `windcube/constants.py`. H05 does not
define any numerical constants — they all flow through from H01's constants
block (H01 §3).

```python
from windcube.constants import (
    NE_WAVELENGTH_1_AIR_M,    # 640.2248e-9 m — primary neon line
    NE_WAVELENGTH_2_AIR_M,    # 638.2991e-9 m — secondary neon line
    NE_INTENSITY_2,            # 0.36 — secondary line relative intensity
    ETALON_GAP_M,              # 20.008e-3 m — ICOS measured gap (d prior)
    ETALON_R_INSTRUMENT,       # 0.53 — FlatSat effective reflectivity
    ALPHA_RAD_PX,              # 1.6e-4 rad/px — Tolansky-recovered plate scale
)
```

**Note:** `FOCAL_LENGTH_M` and `CCD_PIXEL_2X2_UM` are **not** imported here.
`alpha` (rad/px) is the fundamental parameter used by the H01 Airy forward
model, recovered directly by Tolansky. Focal length and pixel pitch are the
physical origin of `alpha` but play no role in H05.

The H01 forward model functions are imported as:
...
INSTRUMENT_DEFAULTS = {
    ...
    # These are the widest bounds H05 should use without user override
...
class FitConfig:
    """
    User-facing configuration for H05 staged calibration inversion.
    ...
    t_init_m:    Optional[float] = None   # None → use ETALON_GAP_M from constants.py
    ...
    alpha_init:   Optional[float] = None   # None → ALPHA_RAD_PX from constants.py
...
`TOLANSKY_NOT_PROVIDED` is a warning, not a fatal flag. H05 can run
without Tolansky using the `windcube/constants.py` defaults, but the
FSR-period ambiguity risk is then the user's responsibility.
...
class CalibrationResult:
    """
    Output of H05 calibration inversion.
    Passed directly to H06 as fixed instrument characterisation.
    All sigma_ and two_sigma_ fields required by S04.
    """
    # --- Fitted parameters (10 total) ---
    # Group A: Tolansky-seeded, fine-tuned with tight bounds
    t_m:      float   # etalon gap, metres
    alpha:    float   # plate scale, rad/px

    # Group B: fitted from fringe shape
    ...
    # --- 1σ standard errors (all 10 parameters) ---
    sigma_t_m:     float
    sigma_alpha:   float
    sigma_R_refl:  float
    sigma_I0:      float
    ...
    epsilon_cal:          float   # fractional order at fringe centre, λ₁ = 640.2248 nm
                                   # = (2 * t_m / NE_WAVELENGTH_1_AIR_M) mod 1
                                   # This is the zero-wind phase reference for H06
...
def _stage0_seed(
    ...
    Else:
        t_init     = ETALON_GAP_M       (from windcube/constants.py — FSR risk)
        alpha_init = ALPHA_RAD_PX       (from windcube/constants.py)
...
- If Tolansky provided: assert `abs(config.tolansky.d_m - ETALON_GAP_M) < 0.1e-3`
  (Tolansky and constants.py prior agree to within 0.1 mm). Log WARNING if they
  disagree by more than 0.02 mm. Do not raise an error — Tolansky wins.
- `I0_init > 0`, `B_init > 0`
...
**Note on t and alpha in Stage 2:** These parameters are still constrained
by the Tolansky-tightened bounds from `FitConfig.resolve()`. They cannot
move far from their Tolansky seeds. The purpose of including them here is to
absorb the small residual error between the Tolansky peak-position estimate
and the full-profile χ² minimum.
...
def fit_calibration_fringe(
    ...
    """
    Run the full H05 staged calibration inversion on a neon FringeProfile.
...
All tests in `tests/test_h05_calibration_inversion.py`.
...
def test_epsilon_cal_computation(synthetic_cal_profile):
    ...
    result = fit_calibration_fringe(synthetic_cal_profile,
                                    FitConfig(tolansky=synthetic_tolansky))
    expected = (2.0 * result.t_m / NE_WAVELENGTH_1_AIR_M) % 1.0
...
def test_round_trip_recovery():
    """
    Synthesise a calibration image with known InstrumentParams (H01).
    Run M03 → Tolansky → H05. Recovered parameters must match to:
      t_m:    < 1e-9 m  (< 0.001 µm)
      R_refl: < 0.01
    ...
    assert abs(cal.t_m - params.t) < 1e-9
    assert abs(cal.R_refl - params.R_eff) < 0.01
...
For noiseless synthetic data from `InstrumentParams()` defaults:

| Quantity | Expected | Notes |
|----------|----------|-------|
| `t_m` recovery | `params.t` ± 1e-9 m | Tolansky seeds to < 1 µm |
| `R_refl` recovery | `params.R_eff` ± 0.01 | |
| `alpha` recovery | `params.alpha` ± 1e-6 rad/px | |
| `epsilon_cal` | `(2*t/λ₁) mod 1` | Exact formula |
| `chi2_reduced` (noiseless) | 0.8–1.5 | |
...
soc_sewell/
├── src/fpi/
│   └── m05_calibration_inversion_2026_05_05.py
└── tests/
    └── test_h05_calibration_inversion.py
```
...
1. Read this entire spec (H05).
2. Read H01 (`docs/specs/H01_airy_forward_model_2026-05-05.md`) in full.
3. Read S04 (output dataclass and uncertainty conventions).
4. Read S12 (FringeProfile) and S13 (Tolansky / TwoLineResult).
5. Read the current `src/fpi/m05_calibration_inversion_*.py` (latest dated version).
6. Read the current `tests/test_*calibration_inversion*.py`.

Report which dated implementation files you found for steps 5 and 6 before
proceeding.
...
```bash
pytest tests/test_m03_annular_reduction*.py tests/test_tolansky*.py -v --tb=short
```

All must pass before proceeding. Stop and report any failures.

**TASK B — Verify constants**

Confirm `windcube/constants.py` exports all constants listed in H05 §4.
In particular confirm:
- `FOCAL_LENGTH_M` is **not** imported by this module (it may exist in
  constants.py but H05 must not import it)
- `ALPHA_RAD_PX` is present and used as the default alpha seed

Report Yes/No for each required constant.

**TASK C — Create new module**

Create `src/fpi/m05_calibration_inversion_2026_05_05.py` by copying the
...
3. In `FitConfig.resolve()` and `_stage0_seed()`: confirm that when
   `tolansky` is None, `t_init` falls back to `ETALON_GAP_M` and
   `alpha_init` falls back to `ALPHA_RAD_PX` (not computed from
   `FOCAL_LENGTH_M / CCD_PIXEL_2X2_UM`).
4. In `CalibrationResult`: add the Group A / Group B comment block
   distinguishing Tolansky-seeded parameters from fringe-shape-fitted ones
   (see §8 above).
5. All other functions, stages, bounds, and tests are **unchanged**.
6. Update all imports of `InstrumentParams` and `build_instrument_matrix`
   to point to the new H01 dated module:
   `from fpi.airy_forward_model import InstrumentParams, build_instrument_matrix, ...`

Module docstring:
```python
"""
Module:      m05_calibration_inversion_2026_05_05.py
Spec:        docs/specs/H05_calibration_inversion_2026-05-05.md
Author:      Claude Code
...
Changes from prior version:
  - FOCAL_LENGTH_M removed from imports; not used in H05.
  - alpha and t documented as Tolansky-seeded (see CalibrationResult group
    comments and FitConfig.resolve() docstring).
  - CalibrationResult field comments updated to reflect Group A / Group B.
  - H01 import paths updated to 2026_05_05 dated module.
  - No algorithmic changes.
"""
```

**TASK D — Run module tests**

```bash
pytest tests/test_h05_calibration_inversion.py -v --tb=short
```

All 8 tests must pass. If any fail, debug and fix before proceeding.
...
**TASK F — Archive old module and commit**

1. Archive the old spec:
   ```bash
   git mv docs/specs/H05_calibration_inversion_2026-04-06.md \
           docs/specs/archive/H05_calibration_inversion_2026-04-06.md
   ```
2. Copy this spec to `docs/specs/H05_calibration_inversion_2026-05-05.md`.
3. Commit:
   ```
   refactor(H05): remove FOCAL_LENGTH_M; clarify t/alpha as Tolansky-seeded
   Implements: H05_calibration_inversion_2026-05-05.md
...
```
TASK A — Prior tests
  m03 + tolansky tests: N/N pass

TASK B — Constants check
  All H05 §4 constants present: Yes / No (list any missing)
  FOCAL_LENGTH_M not imported: Yes / No

TASK C — Module created
  Source file: src/fpi/m05_calibration_inversion_2026_05_05.py
  FOCAL_LENGTH_M removed: Yes / No
  alpha fallback uses ALPHA_RAD_PX: Yes / No
  CalibrationResult group comments added: Yes / No
  H01 import paths updated: Yes / No
