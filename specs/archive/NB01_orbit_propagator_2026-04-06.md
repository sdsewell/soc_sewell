...
**Spec ID:** S06
**Spec file:** `specs/S06_nb01_orbit_propagator_2026-04-06.md`
...
| `pos_eci_x` | float64 | m | ECI position x-component |
| `pos_eci_y` | float64 | m | ECI position y-component |
| `pos_eci_z` | float64 | m | ECI position z-component |
| `vel_eci_x` | float64 | m/s | ECI velocity x-component |
| `vel_eci_y` | float64 | m/s | ECI velocity y-component |
| `vel_eci_z` | float64 | m/s | ECI velocity z-component |
...
pos_eci_m  = df[['pos_eci_x', 'pos_eci_y', 'pos_eci_z']].values   # shape (N, 3), metres
vel_eci_ms = df[['vel_eci_x', 'vel_eci_y', 'vel_eci_z']].values   # shape (N, 3), m/s
...
1. Read this entire spec before writing any code. Also read S01, S02, S03.
2. Create `src/geometry/` as a Python package with `__init__.py` if it
   does not already exist.
3. Create `src/geometry/nb01_orbit_propagator_2026_04_05.py` with the
   module header:
   ```python
   """
...
7. Write `tests/test_s06_nb01_orbit_propagator.py` with tests T1–T8.
8. Run `pytest tests/test_s06_nb01_orbit_propagator.py -v` — all 8 tests
   must pass before committing.
9. Update `src/geometry/__init__.py` to re-export `propagate_orbit`.

**Commit message:**
```
feat(geometry): implement S06 NB01 orbit propagator, 8/8 tests pass
Implements: S06_nb01_orbit_propagator_2026-04-06.md
```
