...
**Spec ID:** H01
**Spec file:** `docs/specs/H01_airy_forward_model_2026-05-05.md`
...
>    encapsulates the lens geometry for the Airy forward model. `FOCAL_LENGTH_M`
>    has been removed from the constants import list (Section 3), from the
>    `InstrumentParams` defaults table (Section 5.2), and from the Task A
>    constants checklist (Section 12). It remains in `windcube/constants.py`
>    for reference but is not used or checked by H01.
> 2. **`alpha` and `t` (etalon gap) are now Fixed-from-Tolansky in the neon
>    calibration parameter table (Section 4.2).** Both parameters are recovered
>    by the Tolansky two-line analysis (S13) before any LM fitting occurs, and
>    are passed into M05 as fixed seeds with tight Tolansky-derived bounds.
>    They are not free parameters of the forward model itself — they are
>    instrument geometry constants that the Tolansky analysis characterises.
>    The fitted instrument parameters (Section 4.2 table) now correctly reflect
>    what M05 actually fits: R_eff, I0, I1, I2, sigma0, sigma1, sigma2, B
>    (8 parameters), with t and alpha entering only as Tolansky priors.
...
and `make_airglow_spectrum()`. This is pure mathematics and has no knowledge
of optimisation strategy.
...
|---|---|---|---|---|
| `t`      | Etalon gap | Fixed seed from Tolansky (fine-tuned with tight bounds) | 20.008e-3 | m |
| `alpha`  | Plate scale | Fixed seed from Tolansky (fine-tuned with tight bounds) | 1.6000e-4 | rad/px |
...
| `t`      | 20.008e-3 m | constants.py (ICOS) | Authoritative ICOS value; Tolansky-recovered prior for M05 |
| `n`      | 1.0 | constants.py | Air gap refractive index |
| `bin_factor` | 2 | Flight config | 2×2 on-chip binning |
| `alpha`  | 1.6000e-4 rad/px | constants.py (ALPHA_RAD_PX, Tolansky-recovered) | Derived from bin_factor if not set; no focal length needed |
...
were added in the 2026-04-29 revision and are unchanged here.
...
def test_ne_forward_model_roundtrip():
    ...
    - Peak amplitude within 3× I0 (Convention B: A absorbs dlam, so peak
      of s is approximately I0·dlam·I_line, which for I_line=I0 and
      dlam ~ 4e-12 m gives a small number — we check non-negativity and
      peak count rather than absolute amplitude)
    """
    from fpi.airy_forward_model import (
    ...
    # Peak detection: threshold above bias
...
│   └── airy_forward_model_2026_05_05.py   ← this module
├── tests/
│   └── test_airy_forward_model_2026-05-05.py
└── docs/specs/
    ├── H01_airy_forward_model_2026-05-05.md   ← this file
    └── archive/
...
Note: `FOCAL_LENGTH_M` is intentionally absent from this list. It may exist
in constants.py for reference, but H01 does not import or use it.

If any of the listed constants are missing, add them from the authoritative
values in Section 3 and commit:
`feat(constants): add missing constants for H01 v2026-05-05`
...
Create `src/fpi/airy_forward_model_2026_05_05.py` by copying the
current implementation (2026_04_29) with no functional changes — the
only differences are the filename and module docstring date. All
functions and classes are identical.

Module docstring:
```python
"""
Module:      airy_forward_model_2026_05_05.py
Spec:        docs/specs/H01_airy_forward_model_2026-05-05.md
Author:      Claude Code
...
Changes from 2026_04_29:
  - FOCAL_LENGTH_M removed from constants imports (not needed; alpha
    encapsulates lens geometry for the Airy forward model).
  - No functional changes to any function or class.
  - Spec ID corrected from S06/H06 to H01 throughout.
"""
```

**TASK C — Copy test file with updated imports**

Create `tests/test_airy_forward_model_2026-05-05.py` by copying
`tests/test_airy_forward_model_2026-04-29.py` and updating the import
line to point to the new dated module. No other changes.

**TASK D — Run tests**

Run: `pytest tests/test_airy_forward_model_2026-05-05.py -v --tb=short`

All 15 tests must pass. Stop and report if any fail.
...
2. Move the old spec to archive:
   ```
   git mv docs/specs/H01_airy_forward_model_2026-04-29.md \
           docs/specs/archive/H01_airy_forward_model_2026-04-29.md
   ```
3. Copy this new spec to `docs/specs/H01_airy_forward_model_2026-05-05.md`.
...
```
refactor(H01): remove FOCAL_LENGTH_M; t and alpha documented as Tolansky-fixed
Implements: H01_airy_forward_model_2026-05-05.md
No functional changes to forward model code.
15/15 tests pass.
```
...
TASK B — Module created
  Source file: src/fpi/airy_forward_model_2026_05_05.py
  Functional changes from 2026_04_29: None / [list any]

TASK C — Test file created
  Tests T1-T15: all present / [list missing]
